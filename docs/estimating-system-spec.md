...
更新日: 2026-08-05
ステータス: 構想・仕様案（実装は Phase 4 以降で段階的に着手）
...
| Phase | 内容 | 受入基準 |
| --- | --- | --- |
| 4 | 基本積算エンジン（一般土木） | 3工種程度（土工・コンクリート工・舗装工など）で「数量→歩掛→単価→直接工事費→共通仮設/現場管理/一般管理費→税→積算書」を一貫計算し、手計算/既存システムと照合一致 |
| 5 | マリコン（港湾）正式化 | 浚渫・ケーソン・基礎捨石で港湾積算基準（令和8年度）の船舶損料・供用係数・拘束費・回航費・海象条件・土質補正を反映 |
