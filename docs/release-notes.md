...
- 同一品目・地域の実単価と動向評価値が混在しないよう、データソース・単位・分類ごとに時系列、各種Export、アラート、品質診断、予測、KPIを分離
- サンプル物価指数のデータソース分類を`official_index`へ是正し、既存時系列にも反映するMigration 021を追加
- Restore drillでAPI imageのMigration件数、公式データソース分類、47都道府県、東京都労務単価の実Readまで検証
